...
class CacheController extends Controller {
  ...
  async _middleware(path, route, req, res, next) {
    ...
    route.cache.strung = crypto.createHash('md5').update(JSON.stringify(req.originalUrl) + JSON.stringify(req.body) + JSON.stringify(req.query) + JSON.stringify(req.params)).digest('hex');
